...
RayCaster::RayCaster(GameObject* parent)
	: Component(parent)
{
...
int RayCaster::CastRay(glm::vec3 startPosition,glm::vec3 direction,float distance)
{
    ...
    PhysicsSystem::getInstance().GetPhysicsWorld()->raycast(rp3d::Ray(ReactMath::glmVecToRP3d(startPosition), ReactMath::glmVecToRP3d(end)),this);
    if(!hits.empty())
    {
